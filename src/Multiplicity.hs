...
sIsProjectingToArgument x (Fun c vars) p =
  let s = Fun c vars
      index = SBV.sFromIntegral $ sMaybeToInt (elemIndex x (sHandBackArgumentsFromTerm s)) + 1
...
sMultiplicity w s t p
  | s == t && not (isVar s) = SBV.ite (sIsNotProjecting t p) w (literal 0)
  | s == t && isVar s = w
  ...
  where help [] _ _ = literal 0
        help (x:xs) i y = SBV.ite (i .== literal y) (sMultiplicity w x t p) (help xs i (y+1))


findIndexTest :: [Term Char Char] -> Term Char Char -> Projection -> Integer -> Integer
findIndexTest (x:xs) s p y = SBV.ite (sIsProjectingToArgument x s p) (y) (findIndexTest xs s p (y+1))
